...
def insert(userId,table_name):
	...
	tables = cur.fetchall()
	createTableSql = "CREATE TABLE "+ table_name +"(`Id` int(11) NOT NULL AUTO_INCREMENT,`UserId` int(11) DEFAULT NULL,"+"`PostId` int(11) DEFAULT NULL,`Tag` varchar(255) DEFAULT NULL,`UpdateTime` datetime DEFAULT NULL,PRIMARY KEY (`Id`)) ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8"
...
def main():
	...
	table_num = 0
	for table in tables:
		if "UserTags" in table[0]:
			tmp_list = re.split('\_',table[0])
			num = int(tmp_list[1])
			if table_num < num:
				table_num = num
				table_name = table[0]
     
	cur.execute("select max(UserId) from " + table_name)
	breakpoint = cur.fetchone()
	 
	'''

	#a = uid_list.index((breakpoint[0],))
	for uid in uid_list:
		table_num = uid[0]/350000 + 1;
		table_name = 'UserTags_'+str(int(table_num))
